...
export default function InboxModal({ open, meUid, onClose }: Props) {
  ...
  const modalDyn = { ...modal, color: hex, border: `2px solid ${hex}`, boxShadow: `0 0 18px ${rgba(0.35)}, inset 0 0 12px ${rgba(0.08)}` } as const;
  const headerDyn = { ...header, textShadow: `0 0 6px ${rgba(0.6)}` };
  const btnDyn = { ...btn, color: hex, border: `2px solid ${hex}`, boxShadow: `0 0 10px ${rgba(0.25)}` } as const;
  const cardDyn = { ...card, border: `1px solid ${rgba(0.35)}`, background: `linear-gradient(180deg, ${rgba(0.05)}, rgba(0,0,0,0) 40%)` } as const;
...
const modal: React.CSSProperties = { width:620, maxWidth:"90vw", maxHeight:"82vh", overflow:"hidden", background:"#000", color:"#00ff6a", border:"2px solid #00ff6a", borderRadius:8, boxShadow:"0 0 18px rgba(0,255,110,0.35), inset 0 0 12px rgba(0,255,110,0.08)", fontFamily: mono, padding:14, position:"relative" };
const header: React.CSSProperties = { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10, fontWeight:700, letterSpacing:1, textShadow:"0 0 6px rgba(0,255,130,0.6)" };
const btn: React.CSSProperties = { background:"transparent", color:"#00ff6a", border:"2px solid #00ff6a", borderRadius:4, fontFamily:mono, padding:"4px 10px", cursor:"pointer", textTransform:"uppercase", letterSpacing:1, boxShadow:"0 0 10px rgba(0,255,110,0.25)" } as const;
const list: React.CSSProperties = { display:"grid", gap:10, maxHeight:"68vh", overflow:"auto", paddingRight:6 };
const card: React.CSSProperties = { border:"1px solid rgba(0,255,110,0.35)", borderRadius:6, padding:10, background:"linear-gradient(180deg, rgba(0,255,80,0.05), rgba(0,0,0,0) 40%)" };
...
const footerRow: React.CSSProperties = { display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:6 };
